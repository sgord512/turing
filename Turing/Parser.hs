...
type PreInstruction = (Pre State, Pre Symbol, Action, State)
...
stateP :: Parser State
stateP =   (State <$> many1 alphaNum)
       <?> "state"           


stateOrHaltP :: Parser State           
stateOrHaltP =   (State <$> (option "H" $ many1 alphaNum))
...
instrP :: Parser (Pre State, Pre Symbol, Action, State)
instrP = (,,,)
         <$> (preStateP <* char ':')
         <*> (spaces *> preSymbolP) 
         <*> (spaces *> string "->" *> spaces *> actionP)
         <*> (spaces *> char ';' *> stateOrHaltP)
         <?> "instruction"         